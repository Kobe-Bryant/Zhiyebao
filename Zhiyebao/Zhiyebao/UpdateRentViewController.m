...
@interface UpdateRentViewController ()
{
    UIImage* rentImage;
    UIImage* sellImage;
    ...
    NSMutableArray* allFacilityIDArray;
...
//获取发布出租房源信息
- (void)rentHouseInfomation;



@end

@implementation UpdateRentViewController
...
- (void)viewDidLoad
{
    ...
    UIImage* image = [UIImage imageNamed:@"closerentImage.png"];
    ...
    [rightButton addTarget:self action:@selector(DismissMethod:)
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    UIImage* submitPostImage = [UIImage imageNamed:@"savepostImage"];
    ...
    saveLable.frame = CGRectMake(140.0, 6.0, 40.0, 30.0);
    saveLable.text = @"保存";
    saveLable.textColor = [UIColor whiteColor];
    saveLable.font =[UIFont systemFontOfSize:17.0];
    [submitButton addSubview:saveLable];
    ...
    dispatch_async(queue, ^{
        ...
        if (result.isSuccess)  {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                selectDic = result.data;
                areaArray = [selectDic objectForKey:AREA_KEY];
                areaNameArray = [selectDic objectForKey:PROJECT_KEY];
                forwardArray = [selectDic objectForKey:FORWARDS_KEY];
                houseTypeArray = [selectDic objectForKey:HOUSE_TYPE_KEY];
                decorationArray = [selectDic objectForKey:DECORATION_KEY];
                facilityArray = [selectDic objectForKey:FACILITY];
                rentYearArray = [selectDic objectForKey:RENTYEAR];
    ...
    dispatch_group_async(group, queue, ^{
        ...
        if (result.isSuccess) {
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                for (checkButton in self.furnitureView.subviews) {
...
{
    Facility *facility = [facilityArray objectAtIndex:button.tag-300];
...
//设置哪些家具是选中状态
- (void)addSelectselectedFacilityIDArray:(House*)house
{
    

    allFacilityIDArray = [NSMutableArray array];

    for (Facility* facility in facilityArray) {
        
        [allFacilityIDArray addObject:facility.facilityID];
        
    }
    
    selectedFacilityIDArray = [NSMutableArray array];

    for (Facility* selectFacility in house.facilityArray) {
        
       [selectedFacilityIDArray addObject:selectFacility.facilityID];
        
    }
    
    for (NSNumber* facilityId in selectedFacilityIDArray) {
        
        NSUInteger index = [allFacilityIDArray indexOfObject:facilityId];
        checkButton = (UIButton*)[self.furnitureView viewWithTag:index+300];
...
- (void)rentHouseInfomation
{
    ...
    dispatch_async(queue, ^{
        ...
        NSLog(@"self.rentHouse.houseID =%@",self.rentHouse.houseID);
        NSLog(@"%@",userId);
        
        Result* result = [House requestOneWithHouseID:self.rentHouse.houseID userID:userId];
        
        if (result.isSuccess) {
        
          House* house = (House*)result.data;
          ...
          dispatch_async(dispatch_get_main_queue(), ^{
              ...
              areaField.text = house.areaTitle;
              areaNameField.text = house.projectTitle;
              numberField.text = house.building;
              floorField.text = house.level;
              direactionField.text = house.forward;
              moneyField.text = house.price;
              typeHouseField.text = house.houseTypeTitle;
              situationField.text = house.decoration;
              yearField.text = house.decorationYear;
              proportionField.text = house.proportion;
              NSString* rentYearText = [[NSUserDefaults standardUserDefaults]objectForKey:RENT_YEAR_TEXT];
              rentYearField.text = rentYearText;
          
          //将id赋值过去
          for (Area* area in areaArray)
          {
                if ([area.title isEqualToString:house.areaTitle])
                {
          ...
          for (Project* project in areaNameArray)
          {
                  
                if ([project.title isEqualToString:house.projectTitle])
                {
         ...
         for (Forward* forward in forwardArray)
         {
              if ([forward.title isEqualToString:house.forward])
              {
         ...
         for ( HouseType* houseType in houseTypeArray)
         {
                  
              if ([houseType.title isEqualToString:house.houseTypeTitle])
              {
         ...
         for (Decoration* decoration in decorationArray) {
                  
              if ([decoration.title isEqualToString:house.decoration])
              {
         ...
         for (  RentYear* rentYear in rentYearArray)
         {
                
           //  if ([rentYear.title isEqualToString:RENT_YEAR_TEXT]) {
            ...
            //  }
              ...
              //设置按钮的选中状态。
              [self addSelectselectedFacilityIDArray:house];
              ...
              dispatch_group_async(group, queue, ^{
                  ...
                  HouseImage* houseImage = [house.houseImageArray lastObject];
                 
                  NSLog(@"%@",houseImage.largeImageURLString);
                  
                  NSURL* imageUrl = [NSURL URLWithString:houseImage.largeImageURLString];
                  
                  NSData* imageData = [NSData dataWithContentsOfURL:imageUrl];
                  
                  UIImage* rentHouseImage = [UIImage imageWithData:imageData];
                  ...
                  for (UIImageView *uploadImageView in self.uploadImageViewArray) {
                      ...
                      if (uploadImageView.image == nil) {
                          NSLog(@"YES");
                          uploadImageView.image = rentHouseImage;
                          break;
                      }
                  }
              });
...
//返回按钮
- (void)DismissMethod:(UIButton*)bt
{
