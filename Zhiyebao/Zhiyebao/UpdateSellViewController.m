...
@interface UpdateSellViewController ()
{
    UILabel* moneyLabel;
...
//获取发布出售房源信息
- (void)sellHouseInfomation;
@end

@implementation UpdateSellViewController
        ...
        if (result.isSuccess)  {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                for (Facility* facility in facilityArray) {
                    //                    NSLog(@"facility title  = %@ facility id = %@",facility.title,facility.facilityID);
...
- (void)sellHouseInfomation
{
    ...
    dispatch_queue_t queue  =  dispatch_queue_create("sellHouseInfomationQueue", NULL);
    ...
    dispatch_async(queue, ^{
        ...
        NSLog(@"self.rentHouse.houseID =%@",self.sellHouse.houseID);
        NSLog(@"%@",userId);
        
        Result* result = [House requestOneWithHouseID:self.sellHouse.houseID userID:userId];
        ...
        if (result.isSuccess) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                dispatch_group_async(group, queue, ^{
                    ...
                    NSLog(@"%@",house.houseImageArray);
                    
                    houseImage = [house.houseImageArray lastObject];
